...
decl : type  { currentType = (TS_entry)$1; /*System.out.println((TS_entry)$1);*/ }  TArray Lid ';' 
      |declStruct 
     ;
...
func : typeRet Fid '(' listaParam ')' '{'  listacmd retorno ';' '}'
     | main
     | IDENTNULL '('  ')' {System.out.println("Detectou");}
     ;


Fid : IDENT   { TS_entry nodo = ts.pesquisa($1);
                  if (nodo != null){ 
                              yyerror("(sem) Funcao >" + $1 + "< jah declarada");
                  }
                   else ts.insert(new TS_entry($1, currentType, currClass)); 
                       }
    ;

typeRet : type { currentType = (TS_entry)$1; System.out.println((TS_entry)$1); }
        | VOID 
        ;
...
IDENTNULL : IDENT { TS_entry nodo = ts.pesquisa($1);
                              if (nodo == null) 
...
type : INT    { $$ = Tp_INT; }
     | DOUBLE { $$ = Tp_DOUBLE; }
     | BOOL   { $$ = Tp_BOOL; }   
     | IDENT  { $$ = null; } // verificar se está na table
                             // e é da classe NomeStruct
     ;
